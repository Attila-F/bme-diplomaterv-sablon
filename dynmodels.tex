...
\begin{itemize}
	...
	\item \emph{Roll} around the longitudinal \textbf{x}-axis, positive right turning
	\item \emph{Pitch} around lateral \textbf{y}-axis, positive right turning
	\item \emph{Yaw} around the vertival \textbf{z}-axis, positive right turning
...
In hydrodynamics, the geometrical extent of the floating body must be considered. A wave traveling beneath the ship acts as a force distributed in space and time, first pushing the bow, then the stern of the ship upwards. However, exploiting the rigidity of the hull, the effects can be simplified to four separate forces.

When the ship is in motion, it encounters certain waves in a different frequency, depending on the relative direction of the waves and the speed of the ship. Of course the frequency of beam waves (perpendicular to the ship) is independent from the speed.

\begin{align}
	\omega_e = \omega-kV \cos (\mu)
\end{align}
